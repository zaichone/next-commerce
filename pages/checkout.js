...
import { CART_RETRIEVE_SUCCESS, ORDER_SET } from '../utils/constant';
...
function Checkout(props) {
  ...
  const [shippingCity, setShippingCity] = useState(dev ? 'San Jose' : '');
  const [shippingStateProvince, setShippingStateProvince] = useState(
    dev ? 'CA' : ''
  );
  const [shippingCountry, setShippingCountry] = useState(dev ? 'US' : '');
  const [shippingOption, setShippingOption] = useState(dev ? 'Emergency' : '');
  // Payment details
